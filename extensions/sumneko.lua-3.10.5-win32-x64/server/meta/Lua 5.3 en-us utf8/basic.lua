...
---
---A global variable (not a function) that holds the global environment (see [§2.2](command:extension.lua.doc?["en-us/53/manual.html/2.2"])). Lua itself does not use this variable; changing its value does not affect any environment, nor vice versa.
---
...
---
---Otherwise, returns three values: the [next](command:extension.lua.doc?["en-us/53/manual.html/pdf-next"]) function, the table `t`, and `nil`, so that the construction
---```lua
---    for k,v in pairs(t) do body end
---```
---will iterate over all key–value pairs of table `t`.
---
---See function [next](command:extension.lua.doc?["en-us/53/manual.html/pdf-next"]) for the caveats of modifying the table during its traversal.
---
---
...
---
---Receives any number of arguments and prints their values to `stdout`, converting each argument to a string following the same rules of [tostring](command:extension.lua.doc?["en-us/53/manual.html/pdf-tostring"]).
---The function print is not intended for formatted output, but only as a quick way to show a value, for instance for debugging. For complete control over the output, use [string.format](command:extension.lua.doc?["en-us/53/manual.html/pdf-string.format"]) and [io.write](command:extension.lua.doc?["en-us/53/manual.html/pdf-io.write"]).
---
---
...
---
---The conversion of strings can result in integers or floats, according to the lexical conventions of Lua (see [§3.1](command:extension.lua.doc?["en-us/53/manual.html/3.1"])). The string may have leading and trailing spaces and a sign.
---
---
...
---
---For complete control of how numbers are converted, use [string.format](command:extension.lua.doc?["en-us/53/manual.html/pdf-string.format"]).
---
---
